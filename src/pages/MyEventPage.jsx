...
function MyEventPage({ onBack, onEdit, onTicketing, onBroadcast, onAnalytics }) {
  ...
  return (
    <div className="my-event-page">
      <header className="my-event-topbar">
        ...
        <div className="my-event-actions"><button type="button" onClick={onEdit}>✎ <span>Edit</span></button><button type="button" onClick={onEdit} aria-label="Event settings">⚙</button></div>
      </header>

      <main className="my-event-main">
        <div className="my-event-hero"><span className="hero-live-pill">● LIVE NOW</span></div>
        <section className="my-event-heading"><span className="live-badge">● LIVE NOW</span><h1>Kampala Business Summit 2026</h1><p>▣ Sat, 25 September 2026 · 9:00 AM – 5:00 PM</p><p>⌖ Serena Hotel, Kampala</p><button type="button" className="go-live-button">▷ Go to Live Event</button></section>

        <section className="my-event-section"><h2>Performance</h2><div className="performance-grid"><div><span>Registered</span><strong>850</strong></div><div><span>Tickets Sold</span><strong>720</strong></div><div><span>Checked In</span><strong>624</strong></div><div><span>Revenue</span><strong>UGX 38.4M</strong></div></div></section>
