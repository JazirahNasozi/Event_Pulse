...
function AnalyticsPage({ onBack, onHome, onEventDetails }) {
  const [range, setRange] = useState('Last 7 days')
  const nextRange = range === 'Last 7 days' ? 'Last 30 days' : 'Last 7 days'

  return (
    <div className="analytics-page">
      <header className="analytics-topbar"><button type="button" onClick={onBack} aria-label="Back to dashboard">←</button><div><h1>Analytics</h1><span>EVENTPULSE 2025 SUMMIT</span></div><button type="button" onClick={onEventDetails} aria-label="Open event details">☷</button></header>
      <main className="analytics-main">
        ...
        <section className="analytics-metric-grid"><Metric label="Registrations" value="850" note="↗ +12% target" tone="blue" /><Metric label="Tickets Sold" value="720" note="90% of 800 cap" tone="indigo" /><Metric label="Attendance" value="624" note="◉ Checked-in" tone="teal" /><Metric label="Gross Revenue" value="38.4M" note="UGX TOTAL" tone="orange" /></section>
        ...
        <section className="analytics-card"><div className="analytics-card-heading"><div><h2>Ticket Performance</h2><p>Breakdown across configured ticket tiers</p></div></div><TicketRow name="Early Bird Tier" tag="Sold Out" amount="100 / 100" percent="100% allocation exhausted" tone="teal" /><TicketRow name="Regular Delegate" tag="Fast Moving" amount="540 / 600" percent="90% reserved · 60 tickets remaining" tone="blue" /><TicketRow name="Executive VIP Access" tag="Limited" amount="80 / 100" percent="80% reserved · 20 tickets remaining" tone="blue" /></section>
        <section className="analytics-card attendance-card"><div className="analytics-card-heading"><div><h2>Registration &amp; Attendance</h2><p>Arrival ratio at Kampala Serena Convention Hall</p></div><strong>73%<small>TURNOUT</small></strong></div><div className="attendance-bar"><i /></div><div className="attendance-legend"><span><b />624<small>Checked In On-site</small></span><span><b />226<small>Expected Arrivals</small></span></div></section>
      ...
      <nav className="analytics-nav"><button type="button" onClick={onHome}><span>⌂</span>Home</button><button type="button" onClick={onEventDetails}><span>▣</span>Events</button><button type="button" onClick={onEventDetails}><span>⌗</span>Check-in</button><button type="button" className="active"><span>▥</span>Analytics</button><button type="button" onClick={onEventDetails}><span>◉</span>Profile</button></nav>
    </div>
  )
}
